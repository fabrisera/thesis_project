...
'''
After initial processing is done, each processed CSV has to be mapped to the year, id, isin combination to ensure that then they can be merged together to create a desired training set

The operation performed is a left join, for every file a missingness indicator column is added if there are missing rows, and all missing rows are set to zeros. This technique will work best with a nonlinear model. 

Another option would be to impute the mean when the predictors are missing. I have to think this through. Perhaps the idea is to chose simple predictors based on the data that is available. 
'''
